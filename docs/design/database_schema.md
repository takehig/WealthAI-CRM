# WealthAI データベーススキーマ設計

## 概要
ウェルスマネジメント向けCRMシステムのデータベース設計
3つのスクリーニングシナリオに対応:
1. 満期債券スクリーニング
2. 政治イベント影響分析
3. 入金予測・履歴分析
...
ALTER TABLE holdings ADD CONSTRAINT fk_holdings_customer 
    FOREIGN KEY (customer_id) REFERENCES customers(customer_id);
ALTER TABLE holdings ADD CONSTRAINT fk_holdings_product 
...
ALTER TABLE transactions ADD CONSTRAINT fk_transactions_product 
    FOREIGN KEY (product_id) REFERENCES products(product_id);
ALTER TABLE transactions ADD CONSTRAINT fk_transactions_sales_rep 
    FOREIGN KEY (sales_rep_id) REFERENCES sales_representatives(rep_id);
```
